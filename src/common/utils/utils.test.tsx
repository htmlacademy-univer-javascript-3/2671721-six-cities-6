...
import { AUTH_TOKEN_KEY_NAME, Path, ResponseCode } from './const';
import {
...
describe('Utility Functions', () => {
  ...
  describe('calculateRatingPercent', () => {
    it('should calculate correct percentage for rating', () => {
      ...
      expect(calculateRatingPercent(4.5)).toBe(90);
      expect(calculateRatingPercent(3)).toBe(60);
      expect(calculateRatingPercent(2.5)).toBe(50);
  ...
  describe('getSortingFunction', () => {
    ...
    it('should return high to low price sorting function', () => {
      const sortFn = getSortingFunction(SortingType.HIGH_TO_LOW);
      const sorted = [...places].sort(sortFn);
    ...
    it('should return low to high price sorting function', () => {
      const sortFn = getSortingFunction(SortingType.LOW_TO_HIGH);
      const sorted = [...places].sort(sortFn);
    ...
    it('should return top rating sorting function', () => {
      const sortFn = getSortingFunction(SortingType.TOP_RATING);
      const sorted = [...places].sort(sortFn);
    ...
    it('should handle empty arrays', () => {
      const sortFn = getSortingFunction(SortingType.HIGH_TO_LOW);
      const sorted: IPlaceCard[] = [].sort(sortFn);
  ...
  describe('Type Guard Functions', () => {
    describe('isResponseCode', () => {
      it('should return true for valid ResponseCode values', () => {
        ...
        expect(isResponseCode(ResponseCode.INTERNAL_SERVER_ERROR)).toBe(true);
        expect(isResponseCode(ResponseCode.UNAUTHORIZED)).toBe(true);
        expect(isResponseCode(ResponseCode.INVALID_REQUEST)).toBe(true);
      });
