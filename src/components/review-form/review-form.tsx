...
import { setReviewError } from '../../store/reviews/reviews-actions';
...
export const ReviewForm: FC<IReviewFormProps> = ({ offerId }) => {
  ...
  const reviewLoading = useAppSelector(getReviewLoading);
  ...
  return (
    <form
      onSubmit={handleSubmitReview} className="reviews__form form" action="#"
      method="post"
    >
      ...
      />
      {reviewError && (
        <p>Failed to send comment. Please try again later!</p>
      )}
      <div className="reviews__button-wrapper">
        ...
        <button
          ...
          type="submit"
          disabled={reviewLoading || !rating || comment.length < 50 || comment.length > 300}
        >
          Submit
        </button>
      </div>
